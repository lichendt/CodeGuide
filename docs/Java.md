...
6.【强制】泛型通配符<? extends T>来接收返回的数据，此写法的泛型集合不能使用add方 法，而<? super T>不能使用 `get` 方法，做为接口调用赋值时易出错。 说明:扩展说一下`PECS(Producer Extends Consumer Super)`
...
5. 【强制】所有没有明确意义的数字、字母等状态或节点判断用识，所有的枚举类型字段必须要有注释，说明每个数据项的用途、意义。
